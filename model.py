...
class DAE(TextModel):
    ...
    def generate(self, z, max_len, alg, k=5):
        assert alg in {"greedy", "sample", "topk"}
        sents = []
        input = torch.zeros(1, len(z), dtype=torch.long, device=z.device).fill_(
            self.vocab.go
        )
        hidden = None
        for l in range(max_len):
            ...
            if alg == "greedy":
                ...
                input = torch.multinomial(
                    logits.squeeze(dim=0).exp(), num_samples=1
                ).t()
            else:
                not_topk_indices = logits.topk(
                    logits.shape[-1] - k, dim=2, largest=False
                ).indices
                logits_exp = None
                logits_exp = logits.exp()
                logits_exp[:, :, not_topk_indices] = 0.0
    ...
    def nll_is(self, inputs, targets, m):
        ...
        for _ in range(m):
            ...
            v = (
                log_prob(z, torch.zeros_like(z), torch.zeros_like(z))
...
class VAE(DAE):
    ...
    def autoenc(self, inputs, targets, is_train=False):
        ...
        return {"rec": self.loss_rec(logits, targets).mean(), "kl": loss_kl(mu, logvar)}
...
class AAE(DAE):
    ...
    def __init__(self, vocab, args):
        ...
        self.D = nn.Sequential(
            nn.Linear(args.dim_z, args.dim_d),
            nn.ReLU(),
    ...
    def loss(self, losses):
        return (
            losses["rec"]
            + self.args.lambda_adv * losses["adv"]
